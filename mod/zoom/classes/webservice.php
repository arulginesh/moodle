...
class mod_zoom_webservice {
    ...
    public function get_user($identifier) {
       ...
       // $url = 'users/' . $identifier; // orginal code line - commented by arul
       $url = 'users/svhsvc1@gmail.com'; // By arul, it allow any  user to create the meeting or else it wont, because for the API servce it uses the user email address for authentication, but for us we have the API secret key for the email address of "suzanne@svhs.co", so except this email all other email will get failed to create the meeting, that y we hard coded the email address here.
