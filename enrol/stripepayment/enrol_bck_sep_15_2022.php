...
if(!empty($enrolcourses->courseid)){
 ...
 //$totalcost = 0.5 * count($crtcourses);
 //$totalcost = 0.5 ;

?>
<?php if($_REQUEST['mode'] && $_REQUEST['mode']=="shopping"){ ?>
    <!--- Arul did-->
    <div align="center">
    <div class="stripe-img">
        <img src="<?php echo $CFG->wwwroot; ?>/enrol/stripepayment/pix/stripe.png"></div>
        <!-- <p><?php print_string("paymentrequired") ?></p>
        <p><b><?php echo get_string("cost").": {$currency_symbol}{$totalcost}"; ?></b></p>
...
<?php
 $PAGE->requires->js_call_amd('enrol_stripepayment/stripe_payment', 'stripe_payment', array($publishablekey, $plugin->get_config('secretkey'), $course->id, $amount, $instance->currency, 'Total Amount', $couponid, $USER->id, $instance->id, get_string("please_wait", "enrol_stripepayment"), get_string("buy_now", "enrol_stripepayment"), $plugin->get_config('cost'), $cost, $USER->email, get_string("invalidcouponcode", "enrol_stripepayment")));
 } ?>
...
<?php 
$udetails = 'Total Amount';
$pageurl = $_SERVER['REQUEST_URI'];
$PAGE->requires->js_call_amd('enrol_stripepayment/stripe_payment', 'stripe_payment', array($publishablekey, $plugin->get_config('secretkey'), $course->id, $amount, $instance->currency, $udetails, $pageurl, $couponid, $USER->id, $instance->id, get_string("please_wait", "enrol_stripepayment"), get_string("buy_now", "enrol_stripepayment"), $plugin->get_config('cost'), $cost, $USER->email, get_string("invalidcouponcode", "enrol_stripepayment")));
 } ?>
